...
- **Deviazione post-pianificazione (commit `ae52258`):** la cattura non usa più `ActivityResultContracts.TakePicture` (camera di sistema) come previsto dal design, ma una schermata dedicata **CameraX** (`CameraCaptureScreen`) perché su MIUI/Xiaomi `TakePicture` salva file corrotti (immagine verde uniforme) senza contenuto. Ciò ha richiesto le dipendenze CameraX 1.3.4 (deviazione dal vincolo "NESSUNA nuova dipendenza oltre ML Kit"), il permesso `CAMERA` nel manifest e la validazione anti-immagine uniforme (`ReceiptImageValidator`). Il risultato viene restituito alla schermata precedente via `savedStateHandle` (chiave `receipt_path`) e l'OCR parte al rientro. Build verificata: `./gradlew testDebugUnitTest assembleDebug` verde.
